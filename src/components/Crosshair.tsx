...
const Crosshair = ({ color = '#146321', containerRef }: CrosshairProps) => {
  ...
  useEffect(() => {
    const handleMouseMove = (ev: MouseEvent) => {
      ...
      // Hide if scrolled or outside bounds
      if (
        bounds.top < -50 ||
      ...
      // Check if over model area (right side of hero section)
      const relativeX = (ev.clientX - bounds.left) / bounds.width;
      if (relativeX > 0.5) {
        setIsOverModel(true);
      } else {
        setIsOverModel(false);
